	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					foreach ( $data['terms'] as $term_slug ) {
						...
						echo '<li class="chosen"><a rel="nofollow" title="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . $cross_icon . ' ' . esc_html( $term->name ) . '</a></li>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a rel="nofollow" title="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '"aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . $cross_icon . sprintf( __( 'Min %s', 'woocommerce' ), wc_price( $min_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price', $base_link );
				/* translators: %s: maximum price */
				echo '<li class="chosen"><a rel="nofollow" title="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . $cross_icon . sprintf( __( 'Max  %s ', 'woocommerce' ), wc_price( $max_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
